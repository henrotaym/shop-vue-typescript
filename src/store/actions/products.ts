...
const actions: ProductsActions & ActionTree<State, State> = {
  [ActionTypes.products.SET_PRODUCTS]({ commit }, payload) {
    commit(MutationTypes.products.SET_PRODUCTS, payload);
  },
  [ActionTypes.products.STORE_PRODUCT]({ commit, state }, payload) {
    commit(MutationTypes.products.SET_PRODUCTS, [
      ...state.products.all,
      payload,
    ]);
  },
  [ActionTypes.products.UPDATE_PRODUCT]({ commit, state }, payload) {
    ...
    commit(MutationTypes.products.SET_PRODUCTS, products);
  },
  [ActionTypes.products.DELETE_PRODUCT]({ commit, state }, payload) {
    const index = state.products.all.findIndex(({ id }) => id === payload.id);
    if (index === -1) return;
    const products = [...state.products.all];
